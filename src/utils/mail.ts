...
export const sendVerificationEmail = async (email: string, token: string) => {
  const verifyUrl = `${env.CLIENT_URL}/auth/new-verification?token=${token}`;
  ...
  const mailOptions = {
    ...
    html: `
      ...
      <body>
        <div class="container">
          <div class="header">
            <a href="${env.CLIENT_URL}" class="logo">♟ ChessHub Online</a>
...
export const sendPasswordResetEmail = async (email: string, token: string) => {
  const resetUrl = `${env.CLIENT_URL}/auth/new-password?token=${token}`;
  const currentYear = new Date().getFullYear();
